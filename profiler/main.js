...
function draw() {
	...
	if (isNaive)
		data[isNaive][population].push(endTime - startTime);
	else
		data[isNaive][population][maxChildren].push(endTime - startTime);
    ...
    if (frame >= frames_per_epoch) {
		...
		if (isNaive) 
			console.log(`Epoch ${epoch} of ${epochs_per_env} for population : ${population}`);
		else
			console.log(`Epoch ${epoch} / ${epochs_per_env} completed for population : ${population}, maxChildren : ${maxChildren}`);
        if (epoch >= epochs_per_env) {
			if (isNaive)
				saveJSON(data, `checkpoint_${population}.json`);
			else
				saveJSON(data, `checkpoint_${population}_${maxChildren}.json`);
            epoch = 0;
